...
"""
    ...
    def test_different_connection_works(self, duckdb_cursor):
        ...
        conn_a = haybarn.connect()
        conn_b = haybarn.connect()
...
result = haybarn.sql("SELECT * FROM obj").fetchall()
...
result = haybarn.sql("SELECT a FROM obj").fetchall()
...
result = haybarn.sql("SELECT a, b FROM obj WHERE a > 3").fetchall()
