...
class TestArrowIntegration:
    ...
    def test_decimals_roundtrip(self, duckdb_cursor):
        ...
        duck_from_arrow = duckdb_cursor.from_arrow(duck_tbl.to_arrow_table())
    ...
    def test_intervals_roundtrip(self, duckdb_cursor):
        ...
        duck_arrow_tbl = duckdb_cursor.table("intervaltbl").to_arrow_table()["a"]
        ...
        duck_tbl_arrow = duckdb_cursor.table("test").to_arrow_table()["a"]
    ...
    def test_null_intervals_roundtrip(self, duckdb_cursor):
        ...
        duckdb_tbl_arrow = duckdb_cursor.table("intervalnulltbl").to_arrow_table()["a"]
    ...
    def test_nested_interval_roundtrip(self, duckdb_cursor):
        ...
        duckdb_tbl_arrow = duckdb_cursor.table("dictionarytbl").to_arrow_table()["a"]
        ...
        # Struct
        query = "SELECT a from (SELECT STRUCT_PACK(a := INTERVAL 1 MONTHS, b := INTERVAL 10 DAYS, c:= INTERVAL 20 SECONDS) as a) as t"  # noqa: E501
        true_answer = duckdb_cursor.sql(query).fetchall()
        from_arrow = duckdb_cursor.from_arrow(duckdb_cursor.sql(query).to_arrow_table()).fetchall()
    ...
    def test_min_max_interval_roundtrip(self, duckdb_cursor):
        ...
        duck_arrow_tbl = duckdb_cursor.table("intervalminmaxtbl").to_arrow_table()["a"]
    ...
    def test_strings_roundtrip(self, duckdb_cursor):
        ...
        for _i in range(1000):
            duckdb_cursor.execute(
                "INSERT INTO  test VALUES ('Matt Damon'),(NULL), ('Jeffffreeeey Jeeeeef Baaaaaaazos'), ('X-Content-Type-Options')"  # noqa: E501
            )
