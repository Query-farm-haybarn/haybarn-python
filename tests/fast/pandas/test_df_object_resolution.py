...
standard_vector_size = haybarn.__standard_vector_size__
...
class TestResolveObjectColumns:
    ...
    def test_map_duplicate(self, duckdb_cursor):
        ...
        with pytest.raises(haybarn.InvalidInputException, match="Map keys must be unique"):
            duckdb_cursor.sql("select * from x").show()

    def test_map_nullkey(self, duckdb_cursor):
        x = pd.DataFrame([[{"key": [None, "a", "b"], "value": [4, 0, 4]}]])
        with pytest.raises(haybarn.InvalidInputException, match="Map keys can not be NULL"):
            converted_col = duckdb_cursor.sql("select * from x").df()
    ...
    def test_structs_of_different_sizes(self, duckdb_cursor):
        ...
        malformed_struct = haybarn.Value({"v1": 1, "v2": 2}, haybarn.struct_type({"v1": int}))
        with pytest.raises(
    ...
    def test_numpy_datetime_int_internally(self, duckdb_cursor):
        ...
        x = pd.DataFrame({"dates": pd.Series(data=data, dtype="object")})
        with pytest.raises(
            haybarn.ConversionException,
            match=re.escape("Conversion Error: Unimplemented type for cast (BIGINT -> TIMESTAMP)"),
        ):
            rel = haybarn.query_df(x, "x", "create table dates as select dates::TIMESTAMP WITHOUT TIME ZONE from x")
    ...
    def test_analyze_sample_too_small(self, duckdb_cursor):
        ...
        with pytest.raises(haybarn.InvalidInputException, match="Failed to cast value: Unimplemented type for cast"):
            res = duckdb_cursor.sql("select * from x").df()
    ...
    def test_numeric_decimal_zero_fractional(self, duckdb_cursor):
        ...
        duckdb_cursor.execute(reference_query)
        reference = duckdb_cursor.sql("select * from tbl").fetchall()
        conversion = haybarn.query_df(decimals, "x", "select * from x").fetchall()
