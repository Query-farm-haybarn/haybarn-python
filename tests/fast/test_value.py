...
import haybarn
from haybarn import InvalidInputException, NotImplementedException
...
class TestValue:
    ...
    def test_float_to_decimal_prevention(self):
        ...
        con = haybarn.connect()
        with pytest.raises(haybarn.ConversionException, match="Can't losslessly convert"):
    ...
    def test_timestamp_sec_not_supported(self, value):
        con = haybarn.connect()
        with pytest.raises(
            haybarn.NotImplementedException, match=r"Conversion from 'datetime' to type .* is not implemented yet"
        ):
            con.execute("select $1", [value]).fetchall()
