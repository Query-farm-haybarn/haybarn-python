...
class TestConnectionClose:
    ...
    def test_open_and_exit(self):
        with pytest.raises(TypeError), haybarn.connect():
    ...
    def test_get_closed_default_conn(self, duckdb_cursor):
        con = haybarn.connect()
        haybarn.set_default_connection(con)
        haybarn.close()

        # 'haybarn.close()' closes this connection, because we explicitly set it as the default
        with pytest.raises(haybarn.ConnectionException, match="Connection Error: Connection already closed"):
            con.sql("select 42").fetchall()

        default_con = haybarn.default_connection()
