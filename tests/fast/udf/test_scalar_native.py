...
class TestNativeUDF:
    def test_default_conn(self):
        ...
        haybarn.create_function("default_conn_passthrough", passthrough, [BIGINT], BIGINT)
        res = haybarn.sql("select default_conn_passthrough(5)").fetchall()
        assert res == [(5,)]
    ...
    def test_return_incorrectly_typed_object(self):
        ...
        with pytest.raises(
            haybarn.InvalidInputException, match="Failed to cast value: Could not convert string 'duckdb' to INT64"
        ):
    ...
    def test_structs(self):
        ...
        con.create_function(
            "append_field",
            add_extra_column,
            [haybarn.struct_type({"a": BIGINT, "b": BIGINT})],
            haybarn.struct_type({"a": BIGINT, "b": BIGINT, "c": BIGINT}),
        )
