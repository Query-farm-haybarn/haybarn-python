...
from haybarn.sqltypes import INTEGER, TIMESTAMP, TINYINT, VARCHAR
from haybarn.value.constant import IntegerValue, Value
...
class TestExpression:
    ...
    def test_column_expression(self):
        ...
        with pytest.raises(haybarn.BinderException, match='Referenced column "d" not found'):
            rel2 = rel.select(column)
    ...
    def test_coalesce_operator(self):
        ...
        with pytest.raises(haybarn.ConversionException, match="Could not convert string 'hello' to INT64"):
            rel2.fetchall()
        ...
        with pytest.raises(haybarn.InvalidInputException, match="Please provide at least one argument"):
    ...
    def test_collate_expression(self):
        ...
        with pytest.raises(haybarn.BinderException, match="collations are only supported for type varchar"):
            rel.select(FunctionExpression("~~", col2, lower_a).collate("NOCASE"))

        with pytest.raises(haybarn.CatalogException, match="Collation with name non-existant does not exist"):
    ...
    def test_lambda_expression(self):
        ...
        with pytest.raises(haybarn.BinderException, match="This scalar function does not support lambdas"):
            rel2 = rel.select(func)
        ...
        with pytest.raises(haybarn.BinderException, match='Referenced column "y" not found in FROM clause'):
            rel2 = rel.select(func)
    ...
    def test_function_expression_aggregate(self):
        ...
        with pytest.raises(
            haybarn.BinderException, match="Binder Error: Aggregates cannot be present in a Project relation!"
        ):
            rel.select(expr)
    ...
    def test_numeric_overflow(self):
        ...
        rel2 = rel.select(expr)
        with pytest.raises(haybarn.OutOfRangeException, match="Overflow in multiplication of INT16"):
            rel2.fetchall()

        val = haybarn.Value(100, TINYINT)
    ...
    def test_empty_in(self, filter_rel):
        ...
        with pytest.raises(
            haybarn.InvalidInputException, match="Incorrect amount of parameters to 'isin', needs at least 1 parameter"
        ):
            expr = expr.isin()
    ...
    def test_aggregate_error(self):
        ...
        with pytest.raises(
            haybarn.InvalidInputException, match="Invalid Input Error: Please provide arguments of type Expression!"
        ):
